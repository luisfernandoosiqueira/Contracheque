...
public class Funcionario {
    ...
    public Funcionario(String nome, int matricula, int quantidadeDependentes, float salarioBase, int producao) throws Exception {
        ...
        if (matricula <= 0) throw new Exception("Matrícula não pode ser <=0");
        this.matricula = matricula;
        
        if (nome.isEmpty()) throw new Exception("Nome não pode estar vazio");
        this.nome = nome;
        
        if (quantidadeDependentes <= 0) throw new Exception("Quantidade de dependentes não pode ser <=0");
        this.quantidadeDependentes = quantidadeDependentes;
        
        if (salarioBase <= 0) throw new Exception("Salário base não pode ser <=0");
        this.salarioBase = salarioBase;
        
        if (producao <= 0) throw new Exception("Produção base não pode ser <=0");
    ...
    public float calcularGratificacao() {
        ...
        if (producao < 1001) return 500;
        
        if (producao <= 2001) return 1250;
    ...
    public float calcularDescontoINSS() {
        ...
        if (calcularSalarioBruto() < 1413) return (float) (calcularSalarioBruto() * 0.07);
        
        if (calcularSalarioBruto() < 2666.69) return (float) (calcularSalarioBruto() * 0.09);
        
        if (calcularSalarioBruto() < 4000.04) return (float) (calcularSalarioBruto() * 0.12);
    ...
    public float calcularDescontoIRPF() {
        ...
        if (calcularSalarioBruto() < 2259.21) return 0;
        
        if (calcularSalarioBruto() < 2826.66) return (float) (calcularSalarioBruto() * 0.075);
      
        if (calcularSalarioBruto() < 3751.06) return (float) (calcularSalarioBruto() * 0.15);
        
        if (calcularSalarioBruto() < 4664.69) return (float) (calcularSalarioBruto() * 0.225);
    ...
    public float calcularDescontoFilhos() {
        ...
        return 123 * quantidadeDependentes;
